...
def load_decay_more():
    model_path = './keras_models/heatmap_decay/new_more.h5'
...
def load_decay_first():
    model_path = './keras_models/heatmap_decay_first/new.h5'
    os.environ['CUDA_VISIBLE_DEVICES'] = '-1'
    ...
    all_outputs= []
    inputs = decoder_inputs
    # other_inputs = Input(shape=(cfg.predict_step_eva, row, col, 1))
    # other_inputs_var = Input(shape=(cfg.predict_step_eva, row, col, 1))
    other_inputs = Input(shape=(cfg.predict_step_eva, row, col, 1))
    other_inputs_var = Input(shape=(cfg.predict_step_eva, row, col, 1))
    num_other = Input(shape=(cfg.predict_step_eva,1))
    pred_inteval = Input(shape=(cfg.predict_step_eva,1))

    # num_users = Input(shape=(1))
    for time_ind in range(cfg.predict_step_eva):
        # print('k0', inputs.shape)
        fut_outputs_sqns0, fut_state_h, fut_state_c = convlstm_decoder([inputs]+states0)
        states0 = [fut_state_h, fut_state_c]
        fut_outputs_sqns1, fut_state_h, fut_state_c = convlstm_decoder1([fut_outputs_sqns0]+states1)
    ...
    # print('all_outputs', all_outputs)
    decoder_outputs = Lambda(lambda x: K.concatenate(x, axis=1))(all_outputs)
    final_shape = Reshape((cfg.predict_step_eva,row*col), input_shape=(cfg.predict_step_eva,row,col,1))
    decoder_outputs = final_shape(decoder_outputs)
...
def load_decay_model():
    ...
    # 2D conv for other users' gt
    others_conv0 = Conv2D(filters=32, kernel_size=(kernel_size,kernel_size), padding='same',
        activation='relu', use_bias=True)
    others_conv1 = Conv2D(filters=64, kernel_size=(kernel_size,kernel_size), padding='same',
        activation='relu', use_bias=True)
    # others_conv2 = Conv2D(filters=1, kernel_size=(kernel_size,kernel_size), padding='same',   # filters=fps
    #     activation='relu', use_bias=True, kernel_initializer='glorot_uniform')
    ...
    for time_ind in range(cfg.predict_step):
        ...
        num_value = expand_dim_layer(num_value)
        num_value = expand_dim_layer(num_value)
        num_value = K.repeat_elements(num_value, rep=64, axis=4)
    ...
    model = Model([encoder_inputs,decoder_inputs,other_inputs,num_other,pred_inteval],decoder_outputs)
    model.load_weights(model_path)
    return model

def load_decay_model_new():
    # arch_path = './arch/non_heatmap/model_architecture.json'
    model_path = './keras_models/heatmap_decay/newnew.h5'
    os.environ['CUDA_VISIBLE_DEVICES'] = '-1'
...
def load_keras_target_model(path=False):
    arch_path = './arch/non_heatmap/model_architecture.json'
    if not path:
