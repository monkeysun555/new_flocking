...
prediction_types = [1,2,3,4]        # 1: LSTM_t 2: LSTM_c 3: TLP 4: co-TLP
num_others = 30
...
def main():
    ...
    for vid in video_list:
        for range_id in range(num_process):
            res_path = './fov_prediction/accuracy_range' + str(range_id) + '.txt'
            with open(res_path, 'r') as fr:
                t = 1
                for line in fr:
                    line = line.split(' ')
                    if t not in type_datas:
                        type_datas[t] = [[] for _ in range(5)]
                    for j in range(5):
                        type_datas[t][j] += [float(line[j])]
                    t += 1

    log_path = './fov_prediction/accuracy.txt'
...
def multi_processes(user_range, range_id, video_range):
    ...
    model = load_decay_model_new()  # to new
    # model = load_decay_more()
    ...
    for key, value in all_loss.items():
        new_format[key] = [[] for _ in range(5)]
    ...
    log_path = './fov_prediction/accuracy_range' + str(range_id) + '.txt'
