...
def collect_prediction_accuracy():
    accuracy_path = './fov_prediction/number.txt'
    plot_datas = []
    count = 0
...
def plot_bar(types_data, odata):
    ...
    ax.legend(loc='upper left', fontsize=20, ncol=2, labelspacing = -0.08, columnspacing = 0.7, framealpha = 0.)
    ...
    ax2.set_yticklabels(np.arange(0, 5, 1), fontsize = 24)
...
def main():
    ...
    p.show()
    input()
    p.savefig('./figures/fov_prediction/num_others.eps', format='eps', dpi=1000, figsize=(30, 5))
    return
