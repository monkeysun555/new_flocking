...
def plot_bar(types_data):
    ...
    print(xlabel_pos)
    ax.legend(loc='upper right', fontsize=22, ncol=2, framealpha = 0, labelspacing = -0.08, columnspacing = 0.7)
    ...
    ax.set_yticklabels([x/100 for x in np.arange(30, 91, 10)], fontsize = 24)
...
def main():
    ...
    p.show()
    input()
    p.savefig('./figures/fov_prediction/accuracy_nss.eps', format='eps', dpi=1000, figsize=(30, 5))
    return
