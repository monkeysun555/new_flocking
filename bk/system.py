...
def main():
    ...
    elif Config.new_prediction_version == 7:
        model = load_decay_model()
        target_model = load_keras_target_model()
        print("load model successfully!")
    # return 
    ...
    while not event_q.empty():
        ...
        if wait_time > 0.0:
            ...
            pred_type = None
            if Config.new_prediction_version == 6 or Config.new_prediction_version == 7:

                # First of all, check target seg/dis
                curr_playing_seg, predicted_seg_idx = user.tmm_check_target_display()
                output_offset = max(predicted_seg_idx - curr_playing_seg - 1, 0)
                ...
                other_mean, other_std = video_server.get_user_fovs_tmm(curr_playing_seg, predicted_seg_idx)
                ...
                if len(other_mean) <= 1:
                    ...
                    if Config.new_prediction_version == 6:
                        ...
                        # prepare model input
                        num_other = np.ones((Config.target_user_pred_len, Config.n_pitch, Config.n_yaw))*len(other_mean)/48/Config.n_pitch/Config.n_yaw
                        ...
                        num_other = np.ones((Config.target_user_pred_len,1))*(len(other_mean)/48)
                        num_other = np.expand_dims(num_other, axis=0)
