...
COOR = 1
OPTIMIZE = 1
RUN_CASE = 4                  # Case 0: unlimited  case 1:limited size, compare cache algo, 2: #tiles curve 3 bits curve 4: limited to 30s and show figure of version 0
...
size_bit_curve_list = [5000, 10000, 20000, 30000, 60000, 120000, 180000] # in MB [2500, ...., 30000]
...
def plot_size_curve_bit(curves1, curves2):
    ...
    p = plt.figure(figsize=(9, 7))
    for i in range(len(curves1)):
        plt.plot(range(1,len(size_bit_curve_list)+1), curves1[i], color=new_palette[i%3], linewidth=2.0, linestyle=line_type[int(i/3)],\
                marker=markers[int(i/3)], markersize=5.0, markeredgewidth=0, label=labels[i])
    plt.plot([0,10], [1,1], '--', linewidth=1.5, color='k')
    ...
    q = plt.figure(figsize=(9, 7))
    for i in range(len(curves2)):
        plt.plot(range(1,len(size_bit_curve_list)+1), curves2[i], color=new_palette[i%3], linewidth=2.0, linestyle=line_type[int(i/3)],\
                marker=markers[int(i/3)], markersize=5.0, markeredgewidth=0, label=labels[i])
    plt.plot([0,10], [1,1], '--', linewidth=1.5, color='k')
    y_axis_upper = float(max(curves2[-1]))*1.3
    y_axis_lower = float(curves2[-1][0])*1.2
    plt.xticks(range(1, len(size_bit_curve_list)+1), [str(x/8000.0) for x in size_bit_curve_list], fontsize=20)
    plt.yticks(np.arange(int(y_axis_lower/0.1)*0.1, 1.01, 0.4), fontsize=20)
    plt.axis([1, len(size_bit_curve_list), y_axis_lower, y_axis_upper])
    plt.ylabel('Network Efficiency', fontsize=20)
    plt.xlabel('Cache Size (GB)', fontsize=20)
    plt.legend(loc='lower right', fontsize=20, ncol=1)
...
def plot_curve_unlimited():
    ...
    plt.yticks(np.arange(0, y_axis_upper, 50000), [x/10000 for x in np.arange(0, y_axis_upper, 50000)], fontsize=20)
...
def plot_bar_unlimited(hitrates, bandwidth_saved_ratios, size_list, trans_ratio):
    ...
    p = plt.figure(figsize=(9, 7))
    plt.bar(position, hitrates, color='none', width=width, edgecolor=new_palette[0], \
                hatch=patterns[0]*4, linewidth=1.0, zorder = 0, label='Hit Rate')
    plt.bar(position, hitrates, color='none', width=width, edgecolor='k', linewidth=0.5)

    
    plt.bar([x + width + pos_gap for x in position], bandwidth_saved_ratios, color='none', width=width, edgecolor=new_palette[1], \
                hatch=patterns[1]*4, linewidth=1.0, zorder = 0, label='Network Efficiency')
    plt.bar([x + width + pos_gap for x in position], bandwidth_saved_ratios, color='none', width=width, edgecolor='k', linewidth=0.5)   
    
    plt.bar([x + 2*width + 2*pos_gap for x in position], trans_ratio, color='none', width=width, edgecolor=new_palette[3], \
                hatch=patterns[2]*4, linewidth=1.0, zorder = 0, label='Transcoding Rate')
    ...
    xticks_pos = [position[group_id]+1.5*width+pos_gap for group_id in range(len(hitrates))]
    ...
    plt.axis([-0.5*width, position[-1]+ 3*pos_gap + 3*width, 0.8, y_axis_upper])
