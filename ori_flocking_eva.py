...
def plot_buffer(buffers, freezes):
    ...
    x_upper = buffers[-2][0]
    freeze_upper = freezes[-1][0] - seg_offset - 1
    ...
    p = plt.figure(figsize=(20,5))
    plt.plot([x*time_factor for x in  new_buffers_time], new_buffers, color='chocolate', label="Buffer Size", linewidth=2, alpha=0.9)
    
    plt.bar([x[0]-seg_offset for x in freezes], [x[1] for x in freezes], color='blue', label="Freeze")

    plt.legend(loc='upper right', fontsize=28, ncol=2)
    plt.xticks(np.arange(0, freeze_upper+1, 50), fontsize=28)
...
def main():
    ...
    data_path = './data/version_2/'
    figure_path = './figures/version_2/user' + str(PLOT_USER) + '/'
    if not os.path.isdir(figure_path):
        os.makedirs(figure_path)
    # Get initial latency
    initial_latency = {}
    system_info_path =  data_path + 'Mapping_usrs48_coor1_latency1.txt'
    line_num = 0
    ...
    fov_time, yaw, pitch = [], [], []
    fov_path = '/home/liyang/Documents/live_360/enhanced_live_360/pickled_data/5/' + str(PLOT_USER) + '.p'
    ...
    p.savefig(figure_path + 'ori_coor' + str(COOR) + '_latency' + str(OPTIMIZE) + '_fov.eps', format='eps', dpi=1000, figsize=(20, 5))
    ...
    p.savefig(figure_path + 'ori_coor' + str(COOR) + '_latency' + str(OPTIMIZE) + '_ratio.eps', format='eps', dpi=1000, figsize=(20, 5))
    ...
    p.savefig(figure_path + 'ori_coor' + str(COOR) + '_latency' + str(OPTIMIZE) + '_buffer.eps', format='eps', dpi=1000, figsize=(20, 5))
    ...
    p.savefig(figure_path + 'ori_coor' + str(COOR) + '_latency' + str(OPTIMIZE) + '_entropys.eps', format='eps', dpi=1000, figsize=(20, 5))
